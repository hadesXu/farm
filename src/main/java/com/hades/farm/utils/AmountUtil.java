...
public class AmountUtil {
    ...
    //卖出一只鸭一只蛋的净利润
    public static BigDecimal profitCalculate(int type){
        ...
        if(type == GoodsType.EGG.getType()){
            ...
            profit = profit.divide(new BigDecimal(Constant.PRODUCE_EGGS)).setScale(2,BigDecimal.ROUND_HALF_DOWN);//除以总共生产的蛋的数量得到单个利润
        }else if(type == GoodsType.DUCK.getType()){
            profit = Constant.DUCK_PRICE.subtract(Constant.DUCK_PRICE);//总收入
            profit = profit.subtract(Constant.DUCK_PRICE.multiply(Constant.SELL_DUCK_RATE));//减手续费
        }
        return profit;
    }
}
