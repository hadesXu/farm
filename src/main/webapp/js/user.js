		...
		if (isNaN(username)) {
			if (username.indexOf("@") >= 0) {
				if (!Utils.isEmail(username)) {
					...
					Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=check_register', 'username=' + $.toJSON(re), check_register_callback, 'POST', 'JSON');
				}
			} else {
...
function is_registered(username) {
	...
	Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=is_registered', 'username=' + username, registed_callback, 'GET', 'TEXT', true, true);
}
 ...
 * 'disabled'; return false; } Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=check_email', 'email=' +
...
function returnToCart(orderId) {
	Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=return_to_cart', 'order_id=' + orderId, returnToCartResponse, 'POST', 'JSON');
}
...
function checkEmail(email, callback) {
	...
	// Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=check_email', 'email=' + email,
...
function checkEmailExist(email, callback) {
	...
	// Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=check_email', 'email=' + email,
	// check_email_callback, 'GET', 'TEXT', true, true);
	$.post('http://www.52wnc.com/mobile/js/user.php?act=check_email', {
		email: email
	}, function(result) {
		if (result == 'ok') {
			document.getElementById('email_notice').innerHTML = msg_can_rg;
...
function checkMobilePhone(mobile, callback) {
	...
	// Ajax.call('http://www.52wnc.com/mobile/js/user.php?act=check_mobile_phone', 'mobile_phone=' + mobileObj,
...
function checkMobilePhoneExist(mobile, callback) {
	...
	$.post('http://www.52wnc.com/mobile/js/user.php?act=check_mobile', {
		mobile_phone: mobile
	}, function(result) {
...
function sendEmailCode(emailObj, emailCodeObj, sendButton) {
	checkEmail(emailObj, function(result) {
		if (result) {
			// 发送邮件
			var url = 'http://www.52wnc.com/mobile/js/user.php?act=send_email_code&XDEBUG_SESSION_START=ECLIPSE_DBGP';
			$.post(url, {
				email: emailObj.val()
			}, function(result) {
				if (result == 'ok') {
...
function sendMobileCode(mobileObj, mobileCodeObj, sendButton) {
	checkMobilePhone(mobileObj, function(result) {
		if (result) {
			// 发送邮件
			var url = 'http://www.52wnc.com/mobile/js/user.php?act=send_mobile_code&XDEBUG_SESSION_START=ECLIPSE_DBGP';
			$.post(url, {
